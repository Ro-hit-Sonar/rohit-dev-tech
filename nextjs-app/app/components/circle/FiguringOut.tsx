...
export default async function FiguringOut() {
  ...
  return (
    <section
      ...
      className="scroll-arc-scope relative overflow-x-clip border-t border-border bg-background px-6 py-28 sm:py-36"
    >
      {/* The arc sits in the section's vertical dead space rather than in a
          column of its own, and bleeds off the right edge so it reads as a mass
          rather than an icon. Held at --ring-hatch weight, so it never competes
          with the text. */}
      <OpenArc
        marker={false}
        className="pointer-events-none absolute -right-24 top-1/2 hidden w-[min(46rem,60vw)] -translate-y-1/2 opacity-60 lg:block"
      />
